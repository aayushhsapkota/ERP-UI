...
const Print = ({
  ...
  const customeWidth = window.innerWidth < 700 ? "300px" : "600px";
  return (
    <div
      ref={componentRef}
      style={
    ...
    >
      ...
      <h2 className={`pt-2 flex items-center justify-between`}>
        ...
        <span className={`text-sm text-[${onlyDate}]`}>
          Date: {convertDate(invoiceForm?.createdAt, true)}
        </span>
      </h2>
      <h2>
        {!isQuotation ? "Bill" : "Quatation"} {sale ? "To" : "From"}:{" "}
        {invoiceForm?.clientDetail?.name || ""}
      </h2>
      <h2>Mobile No: {invoiceForm?.clientDetail?.mobileNo || ""}</h2>
      <h2 className="pb-1">
        Address: {invoiceForm?.clientDetail?.billingAddress || ""}
      </h2>
      ...
      <table className={`ml-auto mt-2 text-[${tableSize}]`}>
        <tbody>
          ...
          {invoiceForm?.taxes?.map((tax, index) =>
            tax?.uniqueTitle !== "Extra Fee" && tax?.uniqueTitle !== "VAT %" ? (
              <tr key={index}>
          ...
          {invoiceForm?.taxes?.map((tax, index) =>
            tax?.uniqueTitle === "Extra Fee" ? (
              <tr key={index}>
                <td className="text-right">Extra Charge </td>
                <td className="text-right pl-10 pt-1">Rs.{tax?.amount}</td>
              </tr>
            ) : null
          )}
