...
function InvoiceTable({ showAdvanceSearch }) {
  ...
  const [searchForm, setSearchForm] = useState({ name: "", anything: "" }); // Bug fixed for searching
    ...
    if (data.length === 0 || page !== pageNumber || page) {
      ...
      dispatch(
        getAllTransaction({
          ...
          onlyDraft: onlyDraft,
          // searchBy: searchBy,
          searchBy: searchForm, //Bug fixed for searching
          filterBy: filterBy,
          sortBy: sortBy,
          date: date,
        })
