...
function ClientEditPaymentModel(props) {
  ...
  const handlerClientValue = useCallback(
    (event, keyName) => {
      ...
      const totalAmountToPay = ClientDetail.totalAmountToPay;
      const amount = parseInt(value);
      const previousAmount = parseInt(editedData.amount);
      if (
        amount > totalAmountToPay + previousAmount + 1 &&
        keyName === "amount"
      ) {
        toast.error(
          `Amount can't be greater than ${totalAmountToPay + previousAmount}`,
          {
            position: "bottom-center",
            autoClose: 2000,
          }
  ...
  useEffect(() => {
    if (editedData !== null) {
      ...
      setIsPaidInCash(editedData?.note.includes("Cash") ? true : false);
    } else {
      setAnimate(false);
    }
  }, [editedData]);
