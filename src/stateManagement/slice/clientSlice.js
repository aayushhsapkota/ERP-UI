...
export const deleteClient = (id, merchant) => async (dispatch) => {
  dispatch(setClientStatus(Status.LOADING));
  try {
    dispatch(setDeleteMerchantId(null));
    dispatch(setDeleteId(null));
    const {
      data: { message },
    } = await api.DeleteClientsAPI(id, merchant);
...
export const clientsSlice = createSlice({
  ...
  reducers: {
    ...
    setDeleteMerchantId: (state, action) => {
      state.deletedMerchantID = action.payload;
    },
...
export const getDeletedMerchantForm = (state) =>
  state.clients.deletedMerchantID;
