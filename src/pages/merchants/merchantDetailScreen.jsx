...
const MerchantDetailScreen = () => {
  ...
  const handleDelete = useCallback(
    (item) => {
      dispatch(setDeleteMerchantId(item._id));
    },
    [dispatch]
  );
  ...
  React.useEffect(() => {
    !viewReport && dispatch(getByTransactionID({ id: id, page: page }));
  }, [id, dispatch, viewReport]);
  React.useEffect(() => {
              ...
              <div className="bg-white rounded-xl p-2">
                <div className="flex flex-wrap p-2">
                  <div className="flex-col flex-1">
                    ...
                    <div className={defaultTdContent}>
                      ...
                      >
                        Rs.{" "}
                        {clientDetail?.totalAmountToPay < 0
                          ? clientDetail?.totalAmountToPay * -1
                  ...
                  {clientDetailArray?.map((item, index) => (
                    <div
                      className="flex-col flex-2 cursor-pointer mt-0.5 mr-4"
                      key={index}
                      onClick={() => {
                        if (item.title === "Map") {
                          window.open(
                ...
                <hr />
                <div
                  className="flex flex-wrap pt-3 gap-2 text-[13px] sm:text-[15px] text-green-700 hover:text-green-900 items-center justify-center cursor-pointer"
                  onClick={() => {
                    setViewReport(true);
                  }}
                >
        ...
        <div className="w-full lg:w-2/6 pl-4 pr-4 sm:pl-4 sm:pr-2">
          ...
          <div className="py-2 px-4 bg-white rounded-xl mt-2">
            <Button
              ...
              block={1}
            >
              <span className="inline-block"> Buy Products </span>
            </Button>
          </div>
        </div>
      </div>
