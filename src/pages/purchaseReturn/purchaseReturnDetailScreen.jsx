...
function PurchaseReturnInvoice(props) {
  ...
  const selectedClient = useSelector(getSelectedMerchant);
  ...
  const saveAs = useCallback(
    ...
    [invoiceForm?.clientDetail?.name, invoiceForm?.statusName]
  );
  ...
  useEffect(() => {
    if (params.id === "new" && invoiceForm !== null) {
      ...
      dispatch(getInvoiceNumber("PurchasesReturn"));
      setInvoiceForm({
        ...invoiceNewForm,
  ...
  if (invoiceForm === null || initLoading === "loading")
    return (
      <div className="p-4">
        <EmptyBar title={"Return detail"} initLoading={initLoading} />
      </div>
    );
