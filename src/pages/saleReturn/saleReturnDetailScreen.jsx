...
function SaleReturnInvoice(props) {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Bishwo Hardwares",
  ...
  useEffect(() => {
    if (params.id === "new" && invoiceForm !== null) {
      ...
      dispatch(getInvoiceNumber("SalesReturn"));
      setInvoiceForm({
        ...invoiceNewForm,
  ...
  if (invoiceForm === null || initLoading === "loading")
    return (
      <div className="p-4">
        <EmptyBar title={"Sale Return detail"} initLoading={initLoading} />
      </div>
    );
